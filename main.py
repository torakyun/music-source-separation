...
def main():
    ...
    if args.use_gan:
        if args.input_D == 'outputs+mix':
            ...
            netD = networks.define_D(args.audio_channels * 4, args.channels, args.netD,
                                    args.n_layers_D, args.norm, args.init_type, args.init_gain)
            netD.to(device)
        elif args.input_D == 'output+mix':
            netD = {}
            for key in ['D_drums', 'D_bass', 'D_other', 'D_vocals']:
    ...
    ##########################################
    if args.use_gan:
        if args.input_D == 'outputs+mix' or args.input_D == 'outputs':
            optimizer_D = th.optim.Adam(netD.parameters(), lr=args.lr, betas=(args.beta1, 0.999))
        elif args.input_D == 'output+mix' or args.input_D == 'output':
            netD = {}
            for key in ['D_drums', 'D_bass', 'D_other', 'D_vocals']:
            optimizer_D[key] = th.optim.Adam(netD[i].parameters(), lr=args.lr, betas=(args.beta1, 0.999))
    ##########################################
    ...
    for epoch in range(len(saved.metrics), args.epochs):
        ...
        saved.metrics.append(train_loss.update({"valid": valid_loss, "best": best_loss, "duration": duration}))
        if args.rank == 0:
